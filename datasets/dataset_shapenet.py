...
class ShapeNet(data.Dataset):
    ...
    def __init__(self, opt, category, subcategory, svr=False, train=True):
        self.opt = opt
        self.num_sample = opt.input_point # if train else 2500
        ...
        if True: #not opt.demo or not opt.use_default_demo_samples:
            ...
            if not os.path.exists(self.cache_file):
                ...
                if self.category != self.subcategory:
                    ...
                    self.subcategory_id = self.category_id

                    # Load csv file
                self.csv_path = os.path.join(self.dataset_path, 'all1.csv')
                if not exists(self.taxonomy_path):
                ...
                self.csv_taxonomy = pd.read_csv(self.csv_path)

                if self.train:
                    self.csv_taxonomy = self.csv_taxonomy[self.csv_taxonomy['split'] == 'train']
                else:
                    self.csv_taxonomy = self.csv_taxonomy[self.csv_taxonomy['split'] != 'train']

                self.csv_taxonomy = self.csv_taxonomy[self.csv_taxonomy['synsetId'].str.contains(self.subcategory_id)]
                for _, dict_class in self.csv_taxonomy.iterrows():
                ...
                print(
                    '    subcategory '
                    + colored(self.subcategory_id, 'yellow')
                    + '  '
    ...
    def init_normalization(self):
        ...
        elif self.opt.normalization == 'shape_unit':
            self.normalization_function = pointcloud_processor.Normalization.normalize_shape_unit_functional
    ...
    def load_point_input(self, path):
        ...
        print(path)
        if ext == 'npy':
            points = np.load(path)#, encoding='bytes', allow_pickle=True)
