...
class Mapping2Dto3D(nn.Module):
    ...
    def __init__(self, opt):
        ...
        print(
            f"New MLP decoder : hidden size {opt.bottleneck_size}, num_layers {opt.num_layers}, activation {opt.activation}")
        ...
        self.conv1.cuda(opt.gpu)
        self.conv2.cuda(opt.gpu)
        self.conv_list = nn.ModuleList(
            [torch.nn.Conv1d(self.hidden_neurons, self.hidden_neurons, 1).cuda(opt.gpu) for i in range(self.num_layers)])

        self.last_conv = torch.nn.Conv1d(self.hidden_neurons, self.dim_output, 1).cuda(opt.gpu)

        self.bn1 = torch.nn.BatchNorm1d(self.bottleneck_size).cuda(opt.gpu)
        self.bn2 = torch.nn.BatchNorm1d(self.hidden_neurons).cuda(opt.gpu)

        self.bn_list = nn.ModuleList([torch.nn.BatchNorm1d(self.hidden_neurons).cuda(opt.gpu) for i in range(self.num_layers)])
